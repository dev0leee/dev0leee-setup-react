...
- **토스트는 변경(post·patch·delete)에 달고 조회(GET)에는 달지 않는다.** 조회는 화면이 곧
  결과라 알릴 게 없다. 변경은 결과가 눈에 안 보일 수 있어 성공·실패를 토스트로 알린다. 이
  토스트는 그 뮤테이션 훅(`queries/`)이 소유한다 ([03-api](./03-api.md) 규칙 5).
- **에러를 폼 필드로 보여야 하면 토스트를 쓰지 않는다.** 로그인처럼 필드 에러가 맞는 경우는
  호출부의 `onError`가 처리한다 ([13-forms](./13-forms.md)).
