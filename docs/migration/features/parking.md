...
| ~~PK-Q2~~  | ~~`history.state.forward` 기반 "상세 복귀 감지"~~ → ✅ **확정(2026-07-31): A안.** 목록을 떠날 때 향한 경로를 **목록별 키**로 저장해 판정한다. `useNavigationType() === 'POP'`은 등록 화면에서 돌아온 경우까지 캐시를 유지해 달라진다 | —          | —         |
...
| ~~PK-Q5~~  | ~~마일리지 한도 제한 단지에서 PK4 라우트를 살려둘지~~ → ✅ **확정: 살려둔다.** 등가 이관 기본값이다(레거시도 라우트만 남아 있다)                                                                                                     | —          | —         |
...
| ~~PK-Q11~~ | ~~`case 'RESERVATION_DATE_INVALID('` 오타~~ → ✅ **확정(2026-07-31): 고친다.** 방문예약 **삭제** 실패 시 서버 원문 대신 `방문예약 기간 설정은 최대 7일입니다.`가 뜬다                                                                | —          | —         |

**진행을 막는 항목은 없다. 결정 3건(PK-Q2·PK-Q5·PK-Q11)은 전부 확정됐다.**
남은 것은 서버·실기기 확인뿐이며 전부 **현행 유지**로 이관한다.
