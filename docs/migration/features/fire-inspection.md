...
**`broken-styles.md` §5에서 "대응 토큰 불명 → 현행 유지, B-Q2 확인"으로 분류된 항목이다.**
소방점검이 이 클래스의 **최대 사용처**다(21개 항목 × 2옵션). → `F-Q11` (B-Q2와 함께 결정)
...
| 항목                                          | 상태                                                                                |
| --------------------------------------------- | ----------------------------------------------------------------------------------- |
| `bg-brand-default-background-brand-secondary` | 🔴 **미생성 클래스** (`broken-styles.md` §5). **이 도메인이 최대 사용처** → `F-Q11` |
...
| F-Q11 | `bg-brand-default-background-brand-secondary`가 미생성이라 **라디오 선택 배경이 없다.** F4는 비활성까지 겹쳐 판독이 어렵다. 어떤 색으로 정하는가 (`broken-styles.md` B-Q2와 함께) | F2a · F4 |
...
| **라디오 선택/미선택 배경** (`F-Q11` 결정 반영 전 스크린샷 대조 필수) |
