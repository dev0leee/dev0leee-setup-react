...
**기본은 등가 이관이다 — 고치지 않는다.** 개선은 `deferred.md`로 간다.

> ✅ **예외 2건 (2026-07-31 사용자 확정).** 아래 §5-1·§5-7은 **고쳐서 옮긴다.**
> 둘 다 "레거시가 이미 깨져 있는 경우"에만 결과가 달라지고 정상 경로는 동일하다.
> `BD-Q11`·`BD-Q4` 참조.
...
> ✅ **확정: 고친다 (2026-07-31).** 키에 `postUuid`를 넣는다. 단독으로 고치면 §B10 수정
> 화면이 깨지므로 **초기값 주입도 함께 고친다**(§5-7 아래 BD-Q11). 두 변경은 한 몸이다.
...
→ 타깃에선 `FormProvider` + RHF로 분해한다 (`tech-mapping.md` 3-3).

> ✅ **확정: 재현하지 않는다 (2026-07-31).** 폼을 **화면 스코프**에 두어 떠나면 항상 비운다.
> 게시판 간 값 누수(소통 → 민원)가 사라지는 대신, 실수로 뒤로 갔다 돌아오면 작성 내용이
> 없어진다. 레거시는 이탈 경로에 따라 남기도 지워지기도 해 **예측 불가능했다.** BD-Q4.
...
| ~~BD-Q4~~  | ~~`boardFormStore` 전역 잔존을 재현할지~~ → ✅ **확정 (2026-07-31): 재현하지 않는다.** 폼을 화면 스코프(RHF `FormProvider`)에 두어 **떠나면 항상 비운다.** 게시판 간 값 누수가 사라지고, 대신 실수로 뒤로 갔다 오면 작성 내용이 없어진다                                               | ✅ 확정    | —         |
...
| ~~BD-Q11~~ | ~~B10·B17 초기값 주입 / 상세 쿼리 키~~ → ✅ **확정 (2026-07-31): 둘 다 고친다.** 쿼리 키에 `postUuid`를 넣고, 초기값을 `onMounted` 1회가 아니라 **데이터 도착 시 `reset()`**으로 반영한다. 정상 경로 화면은 동일하고, 레거시가 빈 폼을 띄우던 경우(캐시 미스·URL 직접 진입)만 채워진다 | ✅ 확정    | —         |
...
**진행을 막는 항목이 없다.** BD-Q4·BD-Q11이 2026-07-31에 확정되면서 4/4 착수 블로커도 사라졌다.
