...
- **경로는 예외 없이 `ROUTE_PATH` 상수로.** 아직 이관되지 않은 도메인의 경로도 상수로 두고
  **`routes.md`의 "아직 화면이 이관되지 않은 경로" 구획에 모은다** (2026-07-30 정정).
  이관된 화면이 이동 대상으로 참조하는 경로가 이미 여러 개라, 문자열로 흩어두면
  그 도메인을 이관할 때 어디를 바꿔야 하는지 찾을 수 없다. 구획에 모아두면 한 파일만 본다
