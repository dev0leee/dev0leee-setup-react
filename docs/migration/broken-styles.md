...
레거시 소스가 쓰는 클래스 중 **25개가 `tailwind.config.js`에 없어 CSS가 생성되지 않는다.**
...
| **효과 없는 죽은 선언** (같은 속성을 뒤 클래스가 덮음)        |    2 | **삭제**                  |
...
같은 CSS 속성을 지정하는 **정상 클래스가 뒤에 또 있어서**, 고쳐도 화면이 바뀌지 않는다.
**의도한 색을 추정할 필요 없이 그냥 지우면 된다.**

| 현재 (깨짐)          | 같은 요소의 유효 클래스                  | 사용처                                                 |
| -------------------- | ---------------------------------------- | ------------------------------------------------------ |
| `bg-bg-deep-blue` ⭐ | `object-cover` (배경 지정 자체가 무의미) | `views/BoardView/BoardPostListItem.vue` 썸네일 `<img>` |
| `bg-deep-blue` ⭐    | `bg-defaults-primary-background-primary` | `views/VisitView/VisitListKioskItem.vue`               |
...
표기를 바꾸면 그 796개를 전부 손봐야 한다. 깨진 25개만 config 표기로 맞추는 쪽이 변경 범위가 작다.
(2차 조사 실측: 추출 토큰 821개 중 정상 796 · 미생성 25)
...
- [ ] 게시글 썸네일·키오스크 카드 — 죽은 선언 삭제 후 **변화가 없는가** (§4)
