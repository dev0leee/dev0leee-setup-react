...
| 클라이언트 상태 | Zustand 5                                   |
| HTTP            | axios (`src/api/client.ts` 인스턴스만 사용) |
...
├── api/          # HTTP 하부구조: client, errors, tokenStore, authChannel
...
├── lib/          # 순수 유틸 (cn 등)
├── mocks/        # MSW 핸들러
└── test/         # 테스트 셋업 & renderWithProviders
```

**의존 방향은 한 방향이다:** `lib`/`config`/`api` → `components` → `features` → `app`.
feature가 다른 feature를 import하지 않는다. 필요하면 공용으로 올린다.
...
각 문서는 **"지금 커뮤니티는"** 절로 시작한다 (2026-07 리서치 기준). 커뮤니티가 합의한 것과
아직 논쟁 중인 것을 구분해 적었고, 논쟁 중인 항목은 **"이 프로젝트의 결정" + 재검토 조건**으로 닫아뒀다.
규칙을 뒤집고 싶으면 그 절의 반대 논거를 근거로 제안한다.
