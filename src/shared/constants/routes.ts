...
export const ROUTE_PATH = {
  ...
  APASS: '/apass',
  /**
   * 관리비 상세 (MF1). **진입은 메인 관리비 카드가 유일하다** — 메뉴에 항목이 없다.
   */
  MANAGEMENT_FEE_DETAIL: '/managementFee/detail',
  /**
   * 관리비 조회 (MF2).
   *
   * 🔴 **진입 경로가 없는 미완성 화면이다** — 데이터가 전부 하드코딩 목업이고
   * 메뉴·카드·딥링크 어디도 이 경로를 가리키지 않는다. `MF-Q1` 확정에 따라
   * 목업째로 이관했고 **진입 경로도 만들지 않는다** (`management-fee.md` §1).
   */
  MANAGEMENT_FEE_INFO: '/managementFee/info',
