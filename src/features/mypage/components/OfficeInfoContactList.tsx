...
export const OfficeInfoContactList = () => {
  ...
  return (
    <div className="border-b-neutral-20 flex flex-col items-start gap-1 self-stretch border-b bg-base-b-white px-5 py-4">
        ...
        <ul className="flex w-full flex-col items-start">
          {officeContactList.map((item, index) => {
            return (
              ...
              >
                <span className="text-neutral-90 pretendard-15SemiBold">{item.name}</span>
                <a
                  className="text-brand-primary-100 flex items-center justify-end gap-[2px] pretendard-15Regular underline underline-offset-4"
                  href={`tel:${item.phone ?? ''}`}
                >
