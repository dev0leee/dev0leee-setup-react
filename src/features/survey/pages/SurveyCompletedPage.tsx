...
/**
 * 설문 완료 (SV4). 레거시 `SurveyCompletedView.vue`(94 LOC) 이식.
 *
 * **회원에게만 닫기·확인 버튼이 보인다.** 비회원(opinion)은 갈 곳이 없어서 화면만 남는다.
 *
 * ⚠️ **두 버튼 모두 `/`로 간다** — `/main`이 아니다. 라우터 가드가 다시 `/main`으로
 * 보내주므로 결과는 같지만 한 번 더 돌아간다 (`vote.md` VT-Q8). 투표 완료 화면과 **문구만 다르고 나머지가 같다.**
 *
 * ⚠️ **직접 URL로 들어오면 "잘못된 접근입니다" 모달이 뜬다.** 모달을 닫으면 메인 앱은
 * `/main`, opinion 앱은 `/`로 간다 — SV6은 같은 상황에서 `/survey/list`로 간다. **3곳이
 * 전부 다르다**(`survey.md` §6-4).
 */
export const SurveyCompletedPage = () => {
  ...
  return (
    <div className="h-full w-full bg-[#F6FAFF]">
      ...
      <div className="space-y-3 px-5 pt-11">
        <h2 className="pretendard-22Bold">설문이 완료되었습니다.</h2>
